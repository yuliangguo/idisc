...
def main_worker(gpu, config: Dict[str, Any], args: argparse.Namespace, ngpus_per_node):
    ...
    for i_d, dataset_name in enumerate(config["data"]["train_dataset"]):
        save_dir = os.path.join(args.base_path, config["data"]["train_data_root"][i_d])
        assert hasattr(
    ...
    save_dir_val = os.path.join(args.base_path, config["data"]["val_data_root"])
    valid_dataset = getattr(custom_dataset, config["data"]["val_dataset"])(
        test_mode=True, base_path=save_dir_val, crop=config["data"]["crop"]
    )
