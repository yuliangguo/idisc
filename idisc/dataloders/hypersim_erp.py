...
class HypersimERPDataset(BaseDataset):
    CAM_INTRINSIC = {
        "ALL": torch.tensor(
            [
                [1 / np.tan(np.pi/1472), 0, 704/2.],
                [0, 1 / np.tan(np.pi/1472), 576/2.],
                [0, 0, 1],
            ]
        )
    }
    min_depth = 0.01
    max_depth = 80
    test_split = "hypersim_test_erp.txt"
    train_split = "hypersim_train_erp.txt"
